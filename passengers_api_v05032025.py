...
# URL модуля "Табло" и "Кассы"
TABLO_API_URL = "http://localhost:8003/v1/flights"
TICKETS_API_URL = "http://localhost:8005/v1/tickets/buy"

# Модель данных для билета (согласована с tickets_api_v05032025.py)
class Ticket(BaseModel):
    ticketId: str
    flightId: str
    passengerId: str
    passengerName: str
    isVIP: bool  # Исправлено с vip на isVIP
    menuType: str
    baggageWeight: int
    status: str
    createdAt: str
    ...
    try:
        ticket_response = requests.post(
            ...
            json={
                ...
                "passengerName": name,
                "flightId": flightId,
                "isVIP": isVIP,  # Убедимся, что поле называется isVIP
                "menuType": menuType,
                "baggageWeight": baggageWeight
            }
        )
...
if __name__ == "__main__":
    uvicorn.run("passengers_api_v05032025:app", host="0.0.0.0", port=8004, reload=True)